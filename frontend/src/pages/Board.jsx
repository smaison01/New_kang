  ...
  return (
    <div className="page">
      ...
      <form className="post-form" onSubmit={handleSubmit}>
        ...
        <input placeholder="작성자" value={form.author}
          onChange={e => setForm({ ...form, author: e.target.value })} />
        <textarea placeholder="내용" rows={4} value={form.content}
          onChange={e => setForm({ ...form, content: e.target.value })} />
        <div className="form-buttons">
          <button type="submit">{editId ? '수정 완료' : '작성'}</button>
          {editId && <button type="button" onClick={() => { setForm(EMPTY_FORM); setEditId(null); }}>취소</button>}
        </div>
      </form>
      ...
      <table className="post-table">
        ...
        <tbody>
          {posts.length === 0 && (
            <tr><td colSpan={5} style={{ textAlign: 'center', color: '#aaa' }}>게시글이 없습니다.</td></tr>
          )}
          {posts.map(post => (
            <tr key={post.id}>
              <td>{post.id}</td>
              <td><span className="title-link" onClick={() => setSelected(post)}>{post.title}</span></td>
              <td>{post.author}</td>
